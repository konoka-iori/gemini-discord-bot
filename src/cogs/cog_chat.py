...
class ChatCog(commands.Cog):
    ...
    def generate_chat_embed(self, ctx: discord.interactions.Interaction, message: str) -> tuple[discord.Embed, discord.Embed]:
        ...
        user_embed.set_author(name=ctx.user.name, icon_url=ctx.user.avatar.url if ctx.user.avatar else None)
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(ChatCog(bot=bot, gemini_api_key=bot.gemini_api_key)) #type: ignore
